...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 30 23:23:44 2023
...
from tensorflow.keras.layers import Input, Embedding, LSTM, Bidirectional, Attention, Dense, GlobalAveragePooling1D, BatchNormalization, Multiply
from tensorflow.keras.models import Model
from tensorflow.keras.preprocessing.text import Tokenizer
from tensorflow.keras.preprocessing.sequence import pad_sequences
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Embedding, Input, Dense, LSTM, Bidirectional, Dropout, Attention
...
# Define a function to create and train a Hierarchical Attention Network (HAN) model
def train_han_model(X_train, y_train, X_test, y_test, max_words, max_seq_length, num_classes, epochs, callbacks=None):
    ...
    word_attention = Attention()([lstm_layer, lstm_layer])
    attended_lstm_word = Multiply()([lstm_layer, word_attention])

    # Sentence-level Attention mechanism
    sentence_attention = Attention()([attended_lstm_word, attended_lstm_word])
    attended_lstm_sentence = Multiply()([attended_lstm_word, sentence_attention])

    # Global attention
    global_attention = layers.GlobalAveragePooling1D()(attended_lstm_sentence)

    output_layer = Dense(64, activation='relu')(global_attention)
...
for i in range(num_runs):
    ...
    # Train the HAN model with the callback
    han_model, tokenizer = train_han_model(X_train, y_train_encoded, X_test, y_test_encoded, max_words, max_seq_length, num_classes, epochs, callbacks=[model_checkpoint])

    # Load the best HAN model
    #best_han_model = tf.keras.models.load_model("best_han_model.h5")
    ...
    # Make predictions
    y_pred_prob = han_model.predict(X_test_pad)
...
performance_df.to_csv('hnn_performance.csv', index=False)
...
# Make predictions
y_pred_prob = best_han_model.predict(X_test_pad)
y_pred = np.argmax(y_pred_prob, axis=1)
...
plt.savefig('hnn_confusion_matrix.png')  # Save the figure as an image
plt.show()
